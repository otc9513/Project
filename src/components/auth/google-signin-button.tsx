...
/**
 * Server Component بسيط - يستخدم next-auth signIn("google") كما كان
 * تمامًا في login/page.tsx الأصلي (بلا أي تغيير في السلوك)، فقط استُخرج
 * هنا ليُستخدَم في صفحتي /login و /register معًا دون تكرار الكود.
 */
export function GoogleSignInButton({ label }: { label: string }) {
  return (
    ...
    >
      <button
        type="submit"
        className="flex w-full items-center justify-center gap-3 rounded-xl border border-gray-300 bg-white px-4 py-3.5 text-base font-medium shadow-sm transition hover:bg-gray-50 active:scale-[0.98] dark:border-gray-700 dark:bg-gray-900 dark:text-white dark:hover:bg-gray-800"
      >
        <GoogleIcon />
        {label}
      </button>
